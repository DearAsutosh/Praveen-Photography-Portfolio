...
export function MasonryGridGallery() {
  ...
  // Preload images and animate loader
  useEffect(() => {
    let loaded = 0;
    ...
    const updateProgress = () => {
      loaded += 1;
      const percent = Math.round((loaded / allImages.length) * 100);
      // Animate the number with GSAP for smoothness
      gsap.to(progressRef, {
        current: percent,
        duration: 0.3,
        onUpdate: () => setLoadingProgress(Math.round(progressRef.current)),
        overwrite: "auto",
      });
      if (loaded === allImages.length) {
        setTimeout(() => setShowLoader(false), 600); // Fade out loader after a short delay
      }
    };
  ...
  // Only show images after loader is gone
  useEffect(() => {
    if (!showLoader) {
      setVisibleImages(allImages.slice(0, loadedCount));
    }
  }, [showLoader, loadedCount]);
  ...
  }, [loadedCount, isLoading, showLoader]);

  // After your infinite scroll useEffect
  useEffect(() => {
    if (!showLoader && isLoading) {
      setIsLoading(false);
    }
  }, [loadedCount, showLoader]);
  ...
  return (
    <div className="w-full px-2 py-8">
      ...
      <AnimatePresence>
        {showLoader && (
          ...
          >
            ...
            >
              <div className="text-6xl font-extrabold text-cyan-400 mb-4 tracking-widest select-none">
                {loadingProgress}%
              </div>
              <div className="w-64 h-3 bg-cyan-900 rounded-full overflow-hidden mb-8">
                <motion.div
              ...
              <div className="text-2xl text-white font-semibold tracking-wider animate-pulse">
                Loading Photos...
              </div>
            </motion.div>
          </motion.div>
      ...
      {/* Only show gallery and infinite scroll loader after initial loader is gone */}
      {!showLoader && (
        <>
          ...
          <AnimatePresence>
            {isLoading && (
              ...
              >
                <div className="text-5xl font-extrabold text-cyan-400 mb-8 animate-pulse">
                  Loading...
                </div>
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    >
                      <path
                        ...
                        strokeWidth={2}
                        d="M12 4v12m0 0l-4-4m4 4l4-4m-9 8h14"
                      />
                    </svg>
                  </button>
                  <motion.img
                  ...
                  <button
                    ...
                    aria-label="Previous"
                    disabled={
                      visibleImages.findIndex((img) => img.id === modalImg.id) === 0
                    }
                    style={{
                      opacity:
                        visibleImages.findIndex((img) => img.id === modalImg.id) ===
                        0
                          ? 0.3
                          : 1,
                    }}
                  ...
                  <button
                    onClick={() => {
                      ...
                      if (idx < visibleImages.length - 1)
                        setModalImg(visibleImages[idx + 1]);
                    }}
